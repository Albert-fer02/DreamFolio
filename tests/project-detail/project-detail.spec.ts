...
test.describe('Project detail — dynamic route', () => {
  test(
    ...
    async ({ page }) => {
      ...
      await expect(page.getByRole('heading', { level: 1, name: 'Digital Public Peru' })).toBeVisible();
      await expect(
        page.getByText(
          'Proyecto de tecnología cívica para explorar información pública y transparencia en Perú, ahora en desarrollo privado.'
        )
      ).toBeVisible();
      await expect(page.getByRole('img', { name: 'Digital Public Peru — civic transparency interface' })).toBeVisible();
    }
  );
